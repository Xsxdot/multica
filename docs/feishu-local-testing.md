...
**飞书机器人（后端读取，见 `server/cmd/server/main.go`）：**

| 变量 | 必填 | 说明 |
|------|------|------|
| `FEISHU_APP_ID` | 接入机器人建议是 | 飞书开放平台应用 **App ID**（一般以 `cli_` 开头） |
| `FEISHU_APP_SECRET` | 是 | 应用 Secret |
| `FEISHU_ENCRYPT_KEY` | 否 | 若在开放平台启用了 **Encrypt Key**（事件加密），需与控制台一致 |
| `FEISHU_VERIFY_TOKEN` | 否 | **HTTP 回调**校验用；当前主线接收走 **长连接**，可为空。若混用 HTTP 回调则需与控制台一致 |

将上述变量写入仓库根目录 **`.env`**（`Makefile` 会通过 `include .env` 导出）。
...
除 `.env` 中的数据库与 `JWT_SECRET` 外，飞书联调需 **`FEISHU_APP_ID` + `FEISHU_APP_SECRET`**。
...
- 日志中若已成功连接 Feishu，会出现类似 **`channel leader: feishu adapter connected`**（且不应有连续 SDK 认证失败）。
...
- **Verification Token** → `FEISHU_VERIFY_TOKEN`（主要用于 HTTP 回调链路）
...
若控制台强制填写加密相关字段，请保证 **`FEISHU_ENCRYPT_KEY` / `FEISHU_VERIFY_TOKEN`** 与控制台完全一致。
...
| 现象 | 处理 |
|------|------|
| 后台无 `feishu adapter connected` | 检查 `FEISHU_APP_ID` / `FEISHU_APP_SECRET`；权限是否已发布；机器人是否已进群 |
| SDK 侧认证失败 | 核对 App Secret 是否粘贴完整；系统时间与 NTP；必要时轮换密钥 |
| Encrypt / Verify 不匹配 | 控制台启用加密时，`FEISHU_ENCRYPT_KEY`、`FEISHU_VERIFY_TOKEN` 必须与开放平台一致 |
