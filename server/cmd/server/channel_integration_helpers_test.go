...
// On a hit (channel_user_binding row exists for (provider, sender_id)) it
...
// provider/chat → workspace. Registers a t.Cleanup so the row is removed
...
func bindChatToWorkspace(t *testing.T, provider, externalChatID string, chatType port.ChatType, workspaceID, boundByUserID string) {
	...
	if _, err := testPool.Exec(ctx, `
		INSERT INTO channel_chat_binding
			(provider, external_chat_id, chat_type, workspace_id, is_primary, bound_by_user_id)
		VALUES ($1, $2, $3, $4, TRUE, $5)
		ON CONFLICT (provider, external_chat_id) DO UPDATE
...
// (provider, externalUserID) → multicaUserID. Registers cleanup.
...
func cleanupChatBindings(ctx context.Context, provider, externalChatID string) {
	_, _ = testPool.Exec(ctx,
		`DELETE FROM channel_chat_binding WHERE provider=$1 AND external_chat_id=$2`,
		provider, externalChatID)
}

func cleanupChannelUserBinding(ctx context.Context, provider, externalUserID string) {
	_, _ = testPool.Exec(ctx,
		`DELETE FROM channel_user_binding WHERE provider=$1 AND external_user_id=$2`,
